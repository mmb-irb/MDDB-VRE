<template>
  <p>Follow and <strong>execute in terminal</strong> the instructions below for uploading your trajectory file(s) <a href="https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html" target="_blank" class="font-weight-black">via Amazon Web Services Command Line Interface (AWS CLI)</a>. The <strong>credentials are temporary</strong> and they will <strong>expire in {{ $globals.expire }}</strong>.</p>
  ...
  <v-textarea
    ...
    @click:append-inner="copyCode('aws', 'file')"
    label="single file"
    v-model="codes.file"
    rows="1"
    auto-grow
  ...
  <v-textarea
    class="my-2"
    append-inner-icon="mdi-content-copy"
    @click:append-inner="copyCode('aws', 'files')"
  ...
  <v-textarea
    class="my-2"
    append-inner-icon="mdi-content-copy"
    @click:append-inner="copyCode('aws', 'folder')"
    label="folder"
